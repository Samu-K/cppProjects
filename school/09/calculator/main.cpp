...
int main() {
    ...
    while ( true ) {
        ...
        transform(command_to_be_executed.begin(),
        ...
        for (auto& cmd : COMMANDS) {
            if (cmd.str == command_to_be_executed) {
                ...
                double left=0;
                string_to_double(pieces.at(1),left);
                double right=0;
                if (!string_to_double(pieces.at(1),left)) {
                    cout << "Error: a non-number operand." << endl;
                    break;
                }
                if (!string_to_double(pieces.at(2),right)) {
                    cout << "Error: a non-number operand." << endl;
                    break;
                }
                cout << cmd.action(left,right) << endl;

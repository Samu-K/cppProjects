...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            std::string studnum = parts.at(1);
            if (student_numbers.find(studnum) == student_numbers.end() ) {
            ...
            std::string new_pnum;
            getline(std::cin,new_pnum);

            if (!is_valid_phone_number(new_pnum)) {
                continue;
            }

            Student* to_change = student_numbers.at(studnum);
            to_change->phone_number = new_pnum;
            user_ids.at(to_change->user_id)->phone_number = new_pnum;
            cout << endl;

            std::ofstream students(file_name);
            for (auto &pair : user_ids) {
                Student stud = *(pair.second);
                students << stud.student_number << ";"
                         << stud.user_id << ";"
                         << stud.name << ";"
                         << stud.phone_number << ";"
                         << stud.email << ";";
